...
namespace NotebookValidator.Web.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        // --- MÉTODO CORREGIDO ---
        // Muestra la página para gestionar roles de un usuario
        public async Task<IActionResult> ManageRoles(string id) // <-- Cambio de 'userId' a 'id'
        {
            var user = await _userManager.FindByIdAsync(id); // <-- Usar 'id'
            if (user == null)
            {
                return NotFound();
            }
            ...
            foreach (var role in _roleManager.Roles.Where(r => r.Name != "Admin"))
            {
        ...
        // Procesa el formulario de asignación de roles
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ManageRoles(ManageUserRolesViewModel model)
        {
            ...
            await _userManager.RemoveFromRolesAsync(user, userRoles.Where(r => r != "Admin"));
